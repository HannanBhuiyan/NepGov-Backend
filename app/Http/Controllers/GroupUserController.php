...
class GroupUserController extends Controller
{
    ...
    public function sendMailToUsers(Request $request)
    {
        ...
        foreach($request->allEmails as $email){
            Mail::send('layouts.template.send_user_survay_view', ['slug' => $request->slug, 'userSurvay' => $userSurvay], function($message) use($email){
                $message->to($email);
                $message->subject('Your Opinion about NepGov');
            });
        }
